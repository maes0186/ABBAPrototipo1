...
<c:if
    ...
    <fieldset class="well">
        ...
        <div class="collapsible-content">
            <div class="col-lg-12">
                <c:if test="${param.casoDevolucion != 1 && param.casoDevolucion != 2}">
                    <fieldset id="fieldSetConceptoCS" <c:out value="${param.disableInput}"/>>
                        <div id="contentItemSolicitado" class="tab-content">
                            <c:if test="${bandejaCS.tipoItem == itemMedicamento}">
                                <div class="form-group form-group-sm">
                                    <label for="cantidadAprobadaCS" class="control-label control-label-sm text-right col-lg-3">${PERIODO_LABEL}</label>
                                    <div class="col-lg-2">
                                        <input class="form-control input-sm" id="periodoAprobadoCS" name="periodoAprobado" readonly="readonly"
                                            placeholder="${PERIODO_LABEL}" maxlength="4" value="${conceptoCS.periodoAprobado}" />
                                    </div>
                                    <label for="diasPeriodoCS" class="control-label control-label-sm text-right col-lg-2">${DIAS_PER_LABEL}</label>
                                    <div class="col-lg-2">
                                        <input class="form-control input-sm" id="diasPeriodoCS" name="diasPeriodo"
                                            placeholder="${DIAS_PER_LABEL}" maxlength="4" value="${conceptoCS.diasPeriodo}"
                                            onchange="calcularPeriodoAprobado()" />
                                    </div>
                                </div>
                                <div class="form-group form-group-sm">
                                    <label for="unidadesAprobadasCS" class="control-label control-label-sm text-right col-lg-3">${UNID_APR_PER_LABEL}</label>
                                    <div class="col-lg-2">
                                        <input class="form-control input-sm" id="unidadesAprobadasCS" name="unidadesAprobadas"
                                            placeholder="${UNID_APR_PER_LABEL}" maxlength="4" value="${conceptoCS.unidadesAprobadas}" />
                                    </div>
                                </div>
                                <div class="form-group form-group-sm">
                                    <label for="numeroEntregasCS" class="control-label control-label-sm text-right col-lg-3"
                                        style="padding-right: 0px;">${NUM_ENTREGAS_LABEL}</label>
                                    <div class="col-lg-2">
                                        <input class="form-control input-sm" id="numeroEntregasCS" name="numeroEntregas"
                                            placeholder="${NUM_ENTREGAS_LABEL}" maxlength="4" value="${conceptoCS.numeroEntregas}"
                                            onchange="calcularPeriodoAprobado()" />
                                    </div>
                                    <div class="col-lg-4">
                            ...
                            <c:if test="${bandejaCS.tipoItem == itemProcedimiento}">
                                <div class="form-group form-group-sm">
                                    <label for="unidadesAprobadasCS" class="control-label control-label-sm text-right col-lg-2">${CANT_APR_LABEL}</label>
                                    <div class="col-lg-2">
                                        <input class="form-control input-sm" id="unidadesAprobadasCS" name="unidadesAprobadas"
                                            placeholder="${CANT_APR_LABEL}" maxlength="4" value="${conceptoCS.unidadesAprobadas}" />
                                    </div>
                                </div>
                            </c:if>
                        </div>
                ...
                <div class="form-group form-group-sm">
                    ...
                    <div class="col-lg-12">
                        <textarea name="justificacion" id="justificacionCS" class="form-control input-sm"
                            <c:if test="${!bandejasParam.editable && !bandejasParam.editableRespuesta}">
                        disabled="disabled"   
                    </c:if>>
                    ${conceptoCS.justificacion}
                </textarea>
                    </div>
                </div>
                <fieldset>
