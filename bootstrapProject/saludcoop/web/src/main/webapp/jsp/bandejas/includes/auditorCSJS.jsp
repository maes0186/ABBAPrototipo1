...
<script>
    $(function() {
        $("#fieldSetConceptoCS").prop("disabled", "disabled");
        $("#justificacionCS").val($.trim($("#justificacionCS").val()));

        $("#btnAceptar").click(function(event) {
            event.preventDefault();

            if(eval('${bandejaCS.tipoItem == itemMedicamento}')) {
                validarCantidadesMedicamento();
            }
            
            if ($("#formInfoSolicitudCS").valid()) {
                var formData = $('#formInfoSolicitudCS').serializeArray();
...
//                 }
    ...
    function calcularPeriodoAprobado() {
        var val1 = $("#numeroEntregasCS").val();
        var val2 = $("#diasPeriodoCS").val();
        $("#periodoAprobadoCS").val(val1 * val2);
    }
    ...
    function validarCantidadesMedicamento() {
        if($("input[name=concepto]:checked").val() == 1) {
            $.ajax({
                ...
                data : $("#formInfoSolicitudCS").serializeArray(),
    ...
    function adicionarRegla(data) {
        ...
        val = $("#numeroEntregasCS").val();
        val2 = $("#diasPeriodoCS").val();
        
        $("#periodoAprobadoCS").rules('add', {
            equalTo : (val * val2).toString()
        });

        $("#unidadesAprobadasCS").rules('add', {
            notEqual : true
        });
    }
    ...
    function procesaryvalidarfechas() {
        ...
        if('${nombreBandeja}' == 'bandejaCSNac') {
        	validarCantidadesMedicamento();
        }
        var valid = true;
        $("#fieldSetConceptoCS input").each(function(i, e) {
            if (!$(e).valid()) {
                valid = false;
            }
        });
...
<c:if test="${bandejaCS.tipoItem == itemMedicamento}">
    <script>
                    function addRules() {
                        var validator = $("#formInfoSolicitudCS").validate();
                        validator.resetForm();

                        if ($("input[name=concepto]:checked").val() == 1) {
                            $("#fieldSetConceptoCS").removeAttr("disabled");
                            $("#periodoAprobadoCS").rules("add", {
                                required : true,
                                digits : true
                            });
                            $("#diasPeriodoCS").rules("add", {
                                required : true,
                                digits : true
                            });
                            $("#unidadesAprobadasCS").rules("add", {
                            ...
                            $("#fieldSetConceptoCS").prop("disabled", "disabled");
                            $("#periodoAprobadoCS").rules("remove");
                            $("#diasPeriodoCS").rules("remove");
                            $("#unidadesAprobadasCS").rules("remove");
                            $("#numeroEntregasCS").rules("remove");
