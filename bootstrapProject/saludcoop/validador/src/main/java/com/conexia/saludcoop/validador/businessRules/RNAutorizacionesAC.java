...
@Component
@Rule(description = "Proceso que actualiza la solicitud de alto costo.")
public class RNAutorizacionesAC extends RNProcess {

    private static Logger LOGGER = LoggerFactory.getLogger(RNAutorizacionesAC.class);
        ...
        try {
            ...
            case SystemConstants.RESPONDER:
                if (EstadoAutorizacion.DEVUELTA_REGIONAL == solItem.getAutorizacion().getEstadoAutorizacion().getId()) {
                    ...
                    role = this.roleRepository.findOne(ConstantesTarget.ROLE_AUDITOR_AC_NAC);
                    solItem.getAutorizacion().setRoleDestinatario(role);
