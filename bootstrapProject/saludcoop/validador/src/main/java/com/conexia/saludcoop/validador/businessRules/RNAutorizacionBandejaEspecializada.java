...
@Component
@Rule(description = "Proceso que actualiza la solicitud de items desde las bandejas de auditor especializado.")
public class RNAutorizacionBandejaEspecializada extends RNProcess {

    private static Logger LOGGER = LoggerFactory.getLogger(RNAutorizacionBandejaEspecializada.class);
    ...
    private boolean validarRegla(HashMap<String, Object> aContext) throws Exception {
        ...
        try {
            ...
            case SystemConstants.DEVOLVER:
                ...
                // Se verifica si la solicitud viene del estado aprobada regional
                if (EstadoAutorizacion.APROBADA_REGIONAL.equals(solItem.getAutorizacion().getEstadoAutorizacion().getId())
                        || EstadoAutorizacion.NEGADA_REGIONAL.equals(solItem.getAutorizacion().getEstadoAutorizacion().getId())
                        || EstadoAutorizacion.RESPUESTA_REGIONAL.equals(solItem.getAutorizacion().getEstadoAutorizacion().getId())) {
