...
import com.conexia.saludcoop.common.entity.transaccional.InformacionTutela;
...
@Component
@Rule(description = "Proceso que actualiza la solicitud de items para tutelas.")
public class RNTutelas extends RNProcess {

    private static Logger LOGGER = LoggerFactory.getLogger(RNTutelas.class);
    ...
    private boolean validarRegla(HashMap<String, Object> aContext) throws Exception {
        ...
        try {
            ...
            if (!Boolean.TRUE.equals(dto.getEsLdf())) {
                if (dto.getConcepto() == SystemConstants.APROBAR || dto.getConcepto() == SystemConstants.ANULAR
                        || dto.getConcepto() == SystemConstants.DEVOLVER) {
                    ...
                    ca.setJustificacion(dto.getJustificacionConcepto());
                }
            }
            ...
            Role role;
            InformacionTutela infoTutela = null;

            if (!Boolean.TRUE.equals(dto.getEsLdf())) {
                if (solItem.getAutorizacion().getInfoTutela() == null) {
                    infoTutela = new InformacionTutela();
                    solItem.getAutorizacion().setInfoTutela(infoTutela);
                } else {
                    infoTutela = solItem.getAutorizacion().getInfoTutela();
                }

                infoTutela.setJustificacionConcepto(dto.getJustificacionConcepto());
                infoTutela.setJustificacionConexidad(dto.getJustificacionConexidad());
                infoTutela.setEsTutelaIntegral(dto.getEsTutelaIntegral());
                infoTutela.setExentaCopago(dto.getExcentaCopagos());
                infoTutela.setNumeroTutela(dto.getNumeroTutela());
                infoTutela.setNumeroFallo(dto.getNumeroFallo());
            }
                ...
                if (user != null) {
                    for (Role r : user.getRoles()) {
                        if (SystemConstants.ROLE_IPS.equals(r.getRole()) || SystemConstants.ROLE_MEDICO.equals(r.getRole())
                            ...
                            // TODO: Validar si se trata de una IPS del piloto para dejarla en estado DEVUELTA IPS
            ...
            case SystemConstants.ESCALAR:
                ...
                if (Boolean.TRUE.equals(dto.getEsLdf())) {
                    infoTutela.setJustificacionConceptoLdf(dto.getJustificacionConcepto());
                }
                ...
                if (solItem.getSolMedicamento() != null) {
                    roleDto = autorizadorManager.obtenerRolPorMedicamento(solItem.getSolMedicamento().getMedicamento().getAltoCosto(),
                            solItem.getSolMedicamento().getMedicamento().getVisibleCtc(), false);
                    ...
                    roleDto = autorizadorManager.obtenerRolPorProcedimiento(solItem.getSolProcedimiento().getProcedimiento()
                            .getNivelAutorizacion(), false, solItem.getSolProcedimiento().getEspecialidad().getCodigo(), solItem
                            .getSolProcedimiento().getProcedimiento().getTipoPPM().getDescripcion());
                } else if(solItem.getSolInsumo() != null){
                    roleDto = autorizadorManager.obtenerRolPorInsumo(
                            solItem.getSolInsumo().getInsumo().getNivelAutorizacion() != null ? solItem.getSolInsumo().getInsumo().getNivelAutorizacion().intValue() : null,
                            solItem.getSolInsumo().getInsumo().getAltoCosto(),
                            solItem.getSolInsumo().getInsumo().getVisibleCtc(), 
                            false,
                            solItem.getSolInsumo().getInsumo().getTipo() != null ? solItem.getSolInsumo().getInsumo().getTipo().getCodigo() : null,
                            solItem.getSolInsumo().getInsumo().getEspecialidades(),
                            solItem.getSuperaTopes());                    
                }
                ...
                if (roleDto.getId().equals(ConstantesTarget.ROLE_AUDITOR_CTC_NAC)
                    ...
                    solItem.getAutorizacion().setEstadoAutorizacion(estadoAutorizacionRepo.findOne(EstadoAutorizacion.PENDIENTE_CTC));
                    ...
                    solItem.getAutorizacion()
                            .setEstadoAutorizacion(estadoAutorizacionRepo.findOne(EstadoAutorizacion.PENDIENTE_ALTO_COSTO));
                } else if (roleDto.getId().equals(ConstantesTarget.ROLE_LINEA_FRENTE)) {
                    solItem.getAutorizacion().setEstadoAutorizacion(estadoAutorizacionRepo.findOne(EstadoAutorizacion.PENDIENTE_TUTELA));
                } else {
                    solItem.getAutorizacion().setEstadoAutorizacion(estadoAutorizacionRepo.findOne(EstadoAutorizacion.AUTORIZADO));
                    solItem.getAutorizacion().setFechaAutorizacion(new Date());
                    ...
                    if (solItem.getSolMedicamento() != null) {
                        ...
                        // Si se aprueba automaticamente al escalar se trata de un medicamento POS, por lo que se genera 1 sola entrega
                        ...
                        aContext.put(ConstantesContexto.INFO_ENTREGAS, Collections.singletonList(infoEntrega));
                    }
                }
                ...
                // Se quita la marca de tutela del item
                solItem.setAplicaTutela(Boolean.FALSE);

                break;

            case SystemConstants.ENVIAR_TUTELA:
                if (solItem.getAutorizacion().getInfoTutela() == null) {
                    infoTutela = new InformacionTutela();
                    solItem.getAutorizacion().setInfoTutela(infoTutela);
                } else {
                ...
                role = this.roleRepository.findOne(ConstantesTarget.ROLE_AUDITOR_TUTELAS);
                solItem.getAutorizacion().setRoleDestinatario(role);
            ...
            rta.setMensajeRespuesta("Error al aprobar");
