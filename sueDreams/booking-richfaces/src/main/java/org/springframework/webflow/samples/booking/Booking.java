...
public class Booking implements Serializable {
	...
	public void validateEnterBookingDetails(ValidationContext context) {
		...
		} else if (creditCardName.trim().equals("")) {
			messages.addMessage(new MessageBuilder().error()
					.source("creditCardName")
					.code("booking.creditCardName.empty").build());
		} else if (creditCard.trim().length() != 16) {
			messages.addMessage(new MessageBuilder().error()
					.source("creditCard").code("booking.creditCard.empty")
					.build());
		}
	}
